...
const CoursesApp = {
  setup() {
    ...
    onMounted(() => {
      ...
      // Привязываем обработчик к Shadow DOM (если есть) или к document
      const root = window.__coursesShadowRoot || document;
...
// ============================================================
// ИНИЦИАЛИЗАЦИЯ С SHADOW DOM
// ============================================================
document.addEventListener('DOMContentLoaded', async () => {
  const hostEl = document.getElementById('courses-app');
  if (!hostEl) return;
  
  // Создаём Shadow DOM
  const shadowRoot = hostEl.attachShadow({ mode: 'open' });
  
  // Путь к CSS (из конфига или относительный)
  const config = window.CoursesAppConfig || {};
  const cssPath = config.cssPath || './courses-shadow.css';
  ...
  try {
    ...
    // Вставляем стили в Shadow DOM
    const styleEl = document.createElement('style');
    styleEl.textContent = cssText;
    shadowRoot.appendChild(styleEl);
    
    // Создаём контейнер для Vue
    const appContainer = document.createElement('div');
    appContainer.id = 'vue-root';
    shadowRoot.appendChild(appContainer);
    
    // Сохраняем ссылку на shadowRoot для обработчиков событий
    window.__coursesShadowRoot = shadowRoot;
    
    // Монтируем Vue в Shadow DOM
    createApp(CoursesApp).mount(appContainer);
    
  } catch (error) {
    console.error('Ошибка инициализации каталога курсов:', error);
    hostEl.innerHTML = '<p style="color: red; padding: 20px;">Ошибка загрузки каталога курсов</p>';
  }
});
